...
// Verificación de la existencia de nit_cc_ase en la tabla asesores
$check_stmt = $mysqli->prepare("SELECT COUNT(*) FROM asesores WHERE nit_cc_ase = ?");
$check_stmt->bind_param('s', $nit_cc_ase);
$check_stmt->execute();
...
if ($count == 0) {
    die("Error: El asesor con NIT/CC $nit_cc_ase no existe en la tabla 'asesores'.");
}
