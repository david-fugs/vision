...
$sheet->getStyle('A2:P2')->applyFromArray(['font' => $boldFontStyle]);
...
$sheet->getStyle('A2:P2')->applyFromArray(['font' => $styleHeader, 'fill' => $styleHeader, 'alignment' => $styleHeader]);

// Definir los encabezados de columna
$sheet->setCellValue('A1', 'NUM CONTRATO ' . $num_con);
$sheet->setCellValue('B1', 'DATOS GENERALES');
$sheet->setCellValue('L1', 'IMPUESTOS INMOBILIARIA');
$sheet->setCellValue('I1', 'IMPUESTOS PROPIETARIO');
...
$sheet->setCellValue('F2', 'ADECUACIONES');
$sheet->setCellValue('G2', 'DEPOSITO');
$sheet->setCellValue('H2', 'AFIANZAMIENTO');
$sheet->setCellValue('I2', 'RTE FUENTE PROPIETARIO');
$sheet->setCellValue('J2', 'RTE ICA PROPIETARIO');
$sheet->setCellValue('K2', 'RTE IVA PROPIETARIO');
$sheet->setCellValue('L2', 'RTE FUENTE INMOBILIARIA');
$sheet->setCellValue('M2', 'RTE ICA INMOBILIARIA');
$sheet->setCellValue('N2', 'RTE IVA INMOBILIARIA');
$sheet->setCellValue('O2', 'PAGADO A');
$sheet->setCellValue('P2', 'PAGO COMISION');
...
while ($row = mysqli_fetch_array($res, MYSQLI_ASSOC)) {
    ...
    $sheet->setCellValue('F' . $rowIndex, $row['adecuaciones']);
    $sheet->setCellValue('G' . $rowIndex, $row['deposito']);
    $sheet->setCellValue('H' . $rowIndex, $row['afianzamiento']);
    $sheet->setCellValue('I' . $rowIndex, $row['rte_fte2_prop']);
    $sheet->setCellValue('J' . $rowIndex, $row['rte_ica2_prop']);
    $sheet->setCellValue('K' . $rowIndex, $row['rte_iva2_prop']);
    $sheet->setCellValue('L' . $rowIndex, $row['rte_fte4_inmobi']);
    $sheet->setCellValue('M' . $rowIndex, $row['rte_ica4_inmobi']);
    $sheet->setCellValue('N' . $rowIndex, $row['ret_iva_valor_pii']);
    $sheet->setCellValue('O' . $rowIndex, $row['pagado_a']);
    $sheet->setCellValue('P' . $rowIndex, Si1No2($row['pago_comision']));
      ...
      $totalAdecuaciones += $row['adecuaciones'];
      $totalDeposito += $row['deposito'];
...
$sheet->setCellValue('F' . $rowIndex, $totalAdecuaciones);
$sheet->setCellValue('G' . $rowIndex, $totalDeposito);
$sheet->setCellValue('H' . $rowIndex, $totalAfianzamiento);
$sheet->setCellValue('I' . $rowIndex, $totalRteFteProp);
$sheet->setCellValue('J' . $rowIndex, $totalRteIcaProp);
$sheet->setCellValue('K' . $rowIndex, $totalRteIvaProp);
$sheet->setCellValue('L' . $rowIndex, $totalRteFteInmobi);
$sheet->setCellValue('M' . $rowIndex, $totalRteIcaInmobi);
$sheet->setCellValue('N' . $rowIndex, $totalRteIvaInmobi);
