...
<body>
    ...
    JOIN pagos as  p ON pr.id_pago = p.id_pago
     WHERE pr.diferencia > 0 AND p.num_con = '$nume_con'
    ";
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        echo '
        <tr>
        <td data-label="PAGO No.">' . $row['id_pago_realizado'] . '</td>
        ...
        <td data-label="EXCEL"><a ><img onclick="enviarPagoParcial(' . $row['id_pago'] . ', ' . $row['num_con']  . ' )" class="pago" src="../../img/pagar.png" width=28 height=28></a></td>
        ';
    }
    echo '</table>
</div>';
